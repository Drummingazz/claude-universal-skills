...
- Full address
- Date, session start time, and session end time. Capture the stated arrival time as well **for the description only**: it is never used to set the calendar block or the reminders.
- Any fee mentioned
- Any timezone stated
...
- Description format (updated 2026-08-11): if an arrival or setup time is stated, the ARRIVE line goes first, ahead of everything else (see the blocking window rule below). Then "Contact:" followed by the site contact's name and phone number as a clickable `tel:` link. Then type of service (fuller narrative detail is fine here, unlike the title category), session schedule, drum count, and any purpose, notes, or access information. Do not include the booking agency's own name or email addresses, and do not include a line noting arrival time is unstated, just state it if known and omit the line if not. Never put NDIS disability or support detail in the title, description only. The confirmed fee goes last, at the very bottom of the description, the final settled figure only, not the negotiation (no offer, counter-offer, or back and forth).
- Blocking window: the event spans **gig time only**, session start through session finish, with no setup, arrival, or pack-down time either side. Agency recommended arrival times (Rhythm Culture state one on every booking) are deliberately disregarded when setting the block. Rule set 2026-08-11, superseding both the 2026-07-15 "arrival or start through to finish" wording and the older 60-minutes-each-side default. If an arrival or setup time is stated, it goes in the description as the **first line**, in the form ARRIVE 8:20am to set up (gig block below is workshop time only). It never affects the block.
- Reminders (updated 2026-08-11): anchored to **gig start, never to arrival**. One email 24 hours before session start. No other email. If session start is at or after 10:00am, also add a single popup 3 hours before session start; if session start is before 10:00am, no same-day reminder at all. Because reminders no longer key off arrival, the ARRIVE line at the top of the description is the only early-arrival cue, so it must always be present when an arrival time is stated.
- Timezone rule: if there is any timezone discrepancy, flag it in BOLD CAPS in both the title and the description.
- **Session clock rule (added 2026-08-11).** Every date and time this skill computes, reads, or writes is Brisbane time, Australia/Brisbane, UTC+10, no daylight saving. Never trust a session date header: a sandbox running UTC reads as the previous day for the whole of Gareth's morning, which silently shifts a gig to the wrong day. Derive today's date explicitly as the Australia/Brisbane date before creating, checking, or logging any event.
