...
3. Reference files by **path only**, never copy file content, **but only when the data lives in a place the next session can open.** The fresh session cannot read this chat. So never hand it a pointer to data that has no retrievable home, see the persistence check in the workflow.
...
3. **Scan (full / ultra).** Once you have the goal, work over the conversation already in context and the files it referenced. Note which connectors or MCPs the work relied on (e.g. Outlook, Gmail, Airtable, HubSpot, Google Calendar), so the handoff can list them for the next session to re-confirm. Lite skips this.
4. **Check every referenced thing is reachable by the reader (critical).** For every piece of work the next session needs, ask: is it saved somewhere that session can open AND that survives into a fresh environment?
   - **A "home" must be persistent.** Four tiers:
     - **Persistent disk** (the user's real/mounted project folder, e.g. `/Users/.../Projects/...` on macOS or `C:\Users\...\Projects\...` on Windows) → survives. Reference the path.
     - **MCP-backed store** (an Airtable base, a Google Drive doc, a Notion page, or an Obsidian vault reached only via a Relay/vault MCP — reachable through a tool call, not a filesystem path) → survives, but **this skill's file tools can't write to it.** Reference the base + table / doc URL / vault-relative path, and if the handoff itself needs to land there, either write it via that store's own MCP (if available this session) or hand the user the finished text and tell them to save it there.
     - **Obsidian, prefer local.** An Obsidian vault is usually connected as a **mounted local folder** (the normal Cowork case), not via MCP. Check first: if the vault path is reachable on disk, treat it as **persistent disk** above and write to it directly with `Write` — do not route it through an MCP. Only use the MCP path when the vault is reachable *solely* via Relay, e.g. a cloud/web session with no local mount.
   ...
   - **In a sandbox (e.g. Claude Cowork), the scratchpad is wiped every session.** Only the mounted real-disk folder, MCP-backed stores, and remote servers persist. So:
     - If a needed asset lives only in the scratchpad or only in this chat → **copy it to the persistent project folder first** (use `Bash` for this — `Write` only creates text, so binary assets like images, PDFs, or archives must be copied with a shell command; for remote-regenerable assets, reference the remote id instead), then point to the new persistent path.
     - If there is no filesystem at all (e.g. claude.ai web) → **embed** the finalized data inline under "Carried-over data" below.
   - **If it TRAVELS (share / another machine / a different folder), persistence is not enough, it must be portable.** The reader does not have your disk, so a `/Users/<you>/...` or `C:\Users\<you>\...` path is useless to them. For each referenced file:
...
5. **Dedupe hard.**
   ...
   - For data that does live in a place the reader can reach: path/identifier only, never copy content.
   - If the next session works in the same folder, point to the folder. Don't enumerate its files.
6. **Anchor the location and access.** Name the working directory (and the project, if relevant) up front, so the fresh session knows where it is operating. Then add an **access preflight**: the next session cannot assume the folder is mounted or that last session's connectors are still live. Instruct it that (a) if it cannot open the referenced paths, it must ask the user to connect/grant access to that folder before doing anything else, never guessing paths or working from memory, and (b) it must confirm the listed connectors are connected and ask the user to connect any that are missing before proceeding. In travels mode, skip your absolute local path, name the project/shared source instead.
7. **Write + print.** Save the handoff to a **persistent** location (the user's real project folder, or an MCP-backed store via its own tool), never a sandbox scratchpad or `/tmp`. Decide the shape:
   - **No persistent disk this session** (e.g. Cowork with no folder connected): do **not** write to the scratchpad — it will be wiped. Either save the handoff into an available MCP-backed store (vault / Drive / Notion) via its own tool, or just **print the full prompt in chat** and tell the user to save it into their project or vault themselves. If you can't tell which path is persistent, ask once.
...
- Access: the files below live in <folder>. If you can't open them, ask me to connect / grant access to that folder first. Do not guess paths or work from memory.
- Connectors needed: <e.g. Outlook, Airtable, HubSpot>   (full + ultra). Confirm each is connected; if any is missing, ask me to connect it before proceeding.
